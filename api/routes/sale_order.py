...
# Expune CRUD, statistici, filtrare după perioadă și agent, exporturi etc.
...
def filter_sale_orders_by_date(
    ...
    cu structura identică celei din endpointul `/stats`.
    """
    ...
    if (
        ...
        str(employee_id).strip() == "" or
        str(employee_id).strip().lower() in ("null", "none", "undefined")
    ):
    ...
    sql = text("""
        ...
        WHERE (:start IS NULL OR TRUNC(so.order_date) >= :start)
          AND (:end IS NULL OR TRUNC(so.order_date) <= :end)
          AND (:emp IS NULL OR so.salesperson_id = :emp)
...
def sales_orders_trends(
    start_date_s: Optional[str] = Query(None, alias="start-date"),
    end_date_s:   Optional[str] = Query(None, alias="end-date"),
    granulatie: int = Query(2, ge=1, le=3),
    db: Session = Depends(get_db),
):
    start_date = _norm_date(start_date_s, "start-date")
    end_date   = _norm_date(end_date_s, "end-date")
    ...
    sql_revenue = text(f"""
        ...
        SELECT NVL(SUM(amount), 0) AS venit FROM items
    """)
    venit_row = _first_mapping(db.execute(sql_revenue, params))
    venit_val = float((venit_row or {}).get("VENIT", 0) or 0)

    sql_closed = text(f"SELECT COUNT(*) AS cnt FROM sale_order so WHERE {where_clause}")
    closed_row = _first_mapping(db.execute(sql_closed, params))
    vanzari_incheiate_val = int((closed_row or {}).get("CNT", 0) or 0)

    if granulatie == 1:
        trunc_mask = "IW"
        label_expr = "TO_CHAR(TRUNC(order_date, 'IW'), 'IYYY-IW')"
        label_key = "nr_saptamana"
    elif granulatie == 2:
        trunc_mask = "MM"
        label_expr = "TO_CHAR(TRUNC(order_date, 'MM'), 'YYYY-MM')"
        label_key = "luna_an"
    else:
        trunc_mask = "YYYY"
        label_expr = "TO_CHAR(TRUNC(order_date, 'YYYY'), 'YYYY')"
        label_key = "an"
    ...
    def g(d, k): return d.get(k) or d.get(k.upper()) or d.get(k.lower())
