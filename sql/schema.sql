...
-- Line items: one car (optional if quoting services first) + many services
CREATE TABLE sale_item (
  item_id     NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  order_id    NUMBER NOT NULL REFERENCES sale_order(order_id) ON DELETE CASCADE,
  item_type   VARCHAR2(10) NOT NULL CHECK (item_type IN ('CAR','SERVICE')),
  vehicle_id  NUMBER REFERENCES vehicle(vehicle_id),
  service_id  NUMBER REFERENCES service_item(service_id),
  qty         NUMBER(6,2) DEFAULT 1 CHECK (qty > 0),
  unit_price  NUMBER(12,2) NOT NULL,
  line_total  NUMBER(14,2) GENERATED ALWAYS AS (qty * unit_price) VIRTUAL,
  -- Ensure exactly one FK is set depending on item_type
  CONSTRAINT chk_item_ref
    CHECK (
      (item_type='CAR'     AND vehicle_id IS NOT NULL AND service_id IS NULL) OR
      (item_type='SERVICE' AND service_id IS NOT NULL AND vehicle_id IS NULL)
    )
);
...
GRANT SELECT, INSERT, UPDATE, DELETE ON sale_item TO sales_role, manager_role;
