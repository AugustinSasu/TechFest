...
('SALES_ANNA', 'Anna Pop', 'SALES', (SELECT dealership_id FROM dealership WHERE name = 'VW North City'), 'SALES_ANNA'),
...
-- Managers
INSERT INTO employee(db_username, full_name, role_code, dealership_id, password) VALUES
('MGR_BOB', 'Bob Ionescu', 'MANAGER', (SELECT dealership_id FROM dealership WHERE name = 'VW North City'), 'MGR_BOB'),
...
SET DEFINE OFF;  
-- in SQL*Plus '&' is used for substitution variables: it will ask for a value for labor
INSERT INTO service_item(service_type, name, description, list_price) VALUES
...
-- Get the order ID above; it's needed for car_sale_item and service_sale_item
DECLARE
  v_order_id NUMBER;
BEGIN
  ...
  VALUES (
    v_order_id,
    (SELECT vehicle_id FROM vehicle WHERE model='Golf' AND ROWNUM = 1),
    22000.00
  );
  ...
  VALUES (
    v_order_id,
    (SELECT service_id FROM service_item WHERE name='Sunroof' AND ROWNUM = 1),
    1,
    1200.00
  );
